...
function Dots() {
  return (
    <div style={{ display:'flex', gap:5, alignItems:'center', padding:'13px 18px' }}>
      {[0,1,2].map(i => (
        <div key={i} style={{ width:7, height:7, borderRadius:'50%', background:'#c8a04a', animation:`pulse 1.3s ease-in-out ${i*.18}s infinite` }}/>
      ))}
...
export default function CandidateInterview() {
  ...
  const stageRef   = useRef(0) // track stageIdx in async context

  // Auto-scroll على كل تغيير
  useEffect(() => {
    bottomRef.current?.scrollIntoView({ behavior: 'smooth' })
  }, [messages, typing, genProfile])
  ...
  async function startInterview() {
    ...
    try {
      ...
      const clean = text.replace('[STAGE_COMPLETE]','').trim()
  ...
  async function processReply(reply, currentMsgs, currentStageIdx) {
    ...
    if (done) {
      const ni = currentStageIdx + 1
      if (ni < STAGES.length) {
        setStageIdx(ni)
        stageRef.current = ni

        // انتقال تلقائي — رسالة إعلان المرحلة الجديدة
        setTimeout(async () => {
          setTyping(true)
          try {
            const nextStage = STAGES[ni]
            const transitionMsg = { role:'user', content: `انتقل إلى مرحلة "${nextStage.ar}" وابدأ بأول سؤال فيها مباشرة.` }
            const allMsgs = [...currentMsgs, aMsg, transitionMsg]
            const sys = INTERVIEW_SYSTEM.replace('{STAGE}', nextStage.id)
            const nextReply = await callChat(allMsgs.map(m => ({ role:m.role, content:m.content })), sys)
            const nextClean = nextReply.replace('[STAGE_COMPLETE]','').trim()
            const stageAnnounce = { role:'assistant', content:`✦ ${nextStage.ar} · ${nextStage.en}` }
            setMessages(p => [...p, stageAnnounce, { role:'assistant', content: nextClean }])
          } catch(e) {
        ...
        await buildProfile([...currentMsgs, aMsg])
      }
    }
  }
  ...
  async function send() {
    ...
    try {
      ...
      const reply = await callChat(nextMsgs.map(m => ({ role:m.role, content:m.content })), sys)
      await processReply(reply, nextMsgs, currentIdx)
    } catch(e) {
      setMessages(p => [...p, { role:'assistant', content:`⚠️ خطأ: ${e.message}` }])
      setTyping(false)
    }
  ...
  async function buildProfile(allMsgs) {
    ...
    try {
      ...
      await fetch('/api/candidates', {
        method: 'POST',
        headers: { 'Content-Type':'application/json' },
        body: JSON.stringify({ profile, userId: u.id }),
      })
    ...
    <div style={{ height:'100vh', display:'flex', flexDirection:'column', background:'#080810', fontFamily:"'Tajawal',sans-serif" }}>
      ...
      <div style={{ padding:'10px 24px 0', background:'#0e0e1a', flexShrink:0 }}>
        <div style={{ display:'flex', gap:5, marginBottom:6 }}>
          {STAGES.map((s,i) => (
            <div key={s.id} style={{ flex:1, height:3, borderRadius:2, background: i<stageIdx?'#c8a04a':i===stageIdx?'#c8a04a':'#252538', transition:'background .5s' }}/>
          ))}
        </div>
        <div style={{ display:'flex', gap:5, paddingBottom:10 }}>
          {STAGES.map((s,i) => (
            <div key={s.id} style={{ flex:1, textAlign:'center', fontSize:9, color:i===stageIdx?'#c8a04a':'#7a7690', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{s.ar}</div>
          ))}
        </div>
      </div>
      ...
      <div style={{ flex:1, overflowY:'auto', padding:'20px 0' }}>
        <div style={{ maxWidth:700, margin:'0 auto', padding:'0 20px', display:'flex', flexDirection:'column', gap:16 }}>
          {messages.map((m,i) => {
            const isStageAnnounce = m.content.startsWith('✦')
            if (isStageAnnounce) return (
            ...
            return (
              <div key={i} style={{ display:'flex', justifyContent:m.role==='user'?'flex-end':'flex-start', alignItems:'flex-end', gap:10 }}>
                ...
                <div style={{
                  ...
                  background: m.role==='user'?'linear-gradient(135deg,#7a5e28,#c8a04a)':'#181828',
                  color: m.role==='user'?'#06060e':'#ede8df',
                  border: m.role==='assistant'?'1px solid #252538':'none',
      ...
      <div style={{ flexShrink:0, padding:'12px 20px 20px', background:'#0e0e1a', borderTop:'1px solid #252538' }}>
        <div style={{ maxWidth:700, margin:'0 auto', display:'flex', gap:10, alignItems:'flex-end', background:'#181828', border:`1px solid ${canSend?'rgba(200,160,74,.35)':'#252538'}`, borderRadius:14, padding:'9px 9px 9px 18px', transition:'border-color .2s' }}>
          <textarea
            ref={inputRef}
            value={input} onChange={onInput} onKeyDown={onKey} rows={1}
          ...
          <button onClick={send} disabled={!canSend} style={{
            ...
            background: canSend?'linear-gradient(135deg,#7a5e28,#c8a04a)':'#252538',
            color: canSend?'#06060e':'#7a7690', fontSize:18, cursor:canSend?'pointer':'default',
