...
export default function InterviewComplete() {
  ...
  const [step, setStep]         = useState('consent') // consent | complete
        ...
        <div style={{ width:'100%', maxWidth:480, background:C.card, border:`1px solid ${C.border}`, borderRadius:20, overflow:'hidden' }}>
          ...
          <div style={{ background:`linear-gradient(135deg,rgba(122,94,40,.2),rgba(200,160,74,.05))`, padding:'32px 28px', textAlign:'center', borderBottom:`1px solid ${C.border}` }}>
            ...
            <p style={{ fontSize:13, color:C.muted, lineHeight:1.7 }}>
              قبل أن نبني ملفك المهني — نحتاج موافقتك على بعض الأمور
            </p>
          </div>
          ...
          <div style={{ padding:'28px' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:14, marginBottom:28 }}>
              ...
              <label style={{ display:'flex', gap:12, alignItems:'flex-start', cursor:'pointer', background:consent1?'rgba(200,160,74,.05)':C.surface, border:`1px solid ${consent1?C.gold:C.border}`, borderRadius:12, padding:16, transition:'all .2s' }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize:12, color:C.muted, lineHeight:1.6 }}>
                    بياناتك محمية ولن تُشارك مع أي جهة بدون إذنك.
                    <Link href="/terms" style={{ color:C.gold, marginRight:4 }}>اقرأ الشروط</Link>
                  </div>
                </div>
              </label>
              ...
              <label style={{ display:'flex', gap:12, alignItems:'flex-start', cursor:'pointer', background:consent2?'rgba(200,160,74,.05)':C.surface, border:`1px solid ${consent2?C.gold:C.border}`, borderRadius:12, padding:16, transition:'all .2s' }}>
                ...
                <div>
                  <div style={{ fontSize:14, fontWeight:600, color:C.text, marginBottom:4 }}>
                    أفهم أن ملفي سيُنشر للشركات فقط بعد الدفع
                  </div>
                  <div style={{ fontSize:12, color:C.muted, lineHeight:1.6 }}>
                    ملفك يبقى خاصاً حتى تختار النشر. يمكنك إخفاؤه أو حذفه في أي وقت.
                  </div>
                </div>
              </label>
            </div>

            <button onClick={proceed} disabled={!consent1 || !consent2} style={{
              width:'100%', padding:'14px', borderRadius:12, border:'none',
              background: consent1 && consent2 ? `linear-gradient(135deg,${C.goldDk},${C.gold})` : C.surface,
              color: consent1 && consent2 ? '#06060e' : C.muted,
              fontSize:15, fontWeight:800, cursor: consent1 && consent2 ? 'pointer' : 'default',
      ...
      {step === 'complete' && (
        <div style={{ width:'100%', maxWidth:480 }}>
          ...
          <div style={{ background:C.card, border:`1px solid ${C.gold}`, borderRadius:20, overflow:'hidden', marginBottom:16 }}>
            ...
            <div style={{ background:`linear-gradient(135deg,rgba(122,94,40,.2),rgba(200,160,74,.05))`, padding:'32px 28px', textAlign:'center', borderBottom:`1px solid ${C.border}` }}>
              ...
              <p style={{ fontSize:14, color:C.muted, lineHeight:1.7 }}>
                مقابلتك مكتملة — خطوة واحدة للانطلاق
              </p>
            </div>
            ...
            <div style={{ padding:'28px' }}>
              ...
              <div style={{ background:C.surface, borderRadius:12, padding:20, marginBottom:24 }}>
                <div style={{ fontSize:13, color:C.gold, fontWeight:700, marginBottom:14, textAlign:'center' }}>
                  مقابل 39 ريال فقط تحصل على:
                </div>
                {[
                  ['📄', 'CV احترافي', 'PDF + Word + صورة عربي وإنجليزي'],
                  ['🏢', 'نشر ملفك للشركات', 'تصلك فرص عمل مناسبة تلقائياً'],
                  ['✅', 'تقديم على الوظائف', 'تقدّم على أي وظيفة معلنة في نخبة'],
                ].map(([icon, title, desc]) => (
              ...
              {/* زر الدفع */}
              <button
                onClick={() => alert('Moyasar — قريباً!')}
                style={{ width:'100%', padding:'15px', borderRadius:12, border:'none', background:`linear-gradient(135deg,${C.goldDk},${C.gold})`, color:'#06060e', fontSize:16, fontWeight:800, cursor:'pointer', fontFamily:"'Tajawal',sans-serif", marginBottom:12 }}>
                ادفع 39 ريال وانطلق ←
              </button>
              ...
              <button
                ...
                style={{ width:'100%', padding:'12px', borderRadius:12, border:`1px solid ${C.border}`, background:'transparent', color:C.muted, fontSize:13, cursor:'pointer', fontFamily:"'Tajawal',sans-serif" }}>
                تصفح ملفي بدون نشر
              </button>
            </div>
          </div>
          ...
          <p style={{ textAlign:'center', fontSize:11, color:C.muted, lineHeight:1.7 }}>
            ملفك محفوظ في حسابك — يمكنك الدفع لاحقاً من لوحة التحكم
          </p>
        </div>
      )}
    </div>
