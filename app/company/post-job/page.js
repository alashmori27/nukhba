...
export default function PostJob() {
  ...
  const [form, setForm]       = useState({ title:'', description:'', requirements:'', location:'', salary_range:'', work_type:'full-time' })
  ...
  async function publishJob() {
    ...
    try {
      const res = await fetch('/api/jobs', {
        method:'POST',
        headers:{'Content-Type':'application/json'},
        body: JSON.stringify({ ...form, questions, company_id: user.id, company_name: user.name })
      })
      const data = await res.json()
  ...
  function updateQuestion(i, val) {
    setQuestions(p => { const n=[...p]; n[i]=val; return n })
  }
  ...
  return (
    <div style={{ minHeight:'100vh', background:C.bg, fontFamily:"'Tajawal',sans-serif", color:C.text }}>
      ...
      <div style={{ maxWidth:700, margin:'0 auto', padding:'32px 24px' }}>
        ...
        {step===1 && (
          <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:16, padding:32 }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:18 }}>
              {[
                ['مسمى الوظيفة *','title','مدير مستودعات — محاسب مالي'],
              ].map(([label,key,ph]) => (
                <div key={key}>
                  <label style={{ fontSize:12, color:C.muted, marginBottom:6, display:'block' }}>{label}</label>
                  <input value={form[key]} onChange={e => updateForm(key,e.target.value)} placeholder={ph}
              ...
              {[
                ['وصف الوظيفة *','description','اشرح طبيعة العمل والمهام الرئيسية...'],
                ['المتطلبات والمؤهلات *','requirements','سنوات الخبرة، الشهادات، المهارات المطلوبة...'],
              ].map(([label,key,ph]) => (
                <div key={key}>
                  <label style={{ fontSize:12, color:C.muted, marginBottom:6, display:'block' }}>{label}</label>
                  <textarea value={form[key]} onChange={e => updateForm(key,e.target.value)} placeholder={ph} rows={3}
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
                <div>
                  ...
                  <input value={form.location} onChange={e => updateForm('location',e.target.value)} placeholder="الرياض — جدة..."
                    style={{ width:'100%', background:C.surface, border:`1px solid ${C.border}`, borderRadius:10, padding:'11px 16px', color:C.text, fontFamily:"'Tajawal',sans-serif", fontSize:14 }}/>
                </div>
                <div>
                  <label style={{ fontSize:12, color:C.muted, marginBottom:6, display:'block' }}>الراتب</label>
                  <input value={form.salary_range} onChange={e => updateForm('salary_range',e.target.value)} placeholder="٨٠٠٠ — ١٢٠٠٠ ريال"
                    style={{ width:'100%', background:C.surface, border:`1px solid ${C.border}`, borderRadius:10, padding:'11px 16px', color:C.text, fontFamily:"'Tajawal',sans-serif", fontSize:14 }}/>
                </div>
              </div>
              <div>
                ...
                <div style={{ display:'flex', gap:10 }}>
                  {[['full-time','دوام كامل'],['part-time','دوام جزئي'],['remote','عن بُعد']].map(([val,label]) => (
                    <button key={val} onClick={() => updateForm('work_type',val)} style={{ padding:'8px 18px', borderRadius:20, fontSize:13, cursor:'pointer', fontFamily:"'Tajawal',sans-serif", border:`1px solid ${form.work_type===val?C.gold:C.border}`, background:form.work_type===val?'rgba(200,160,74,.1)':'transparent', color:form.work_type===val?C.gold:C.muted }}>{label}</button>
                  ))}
                </div>
              </div>
              <button onClick={generateQuestions} disabled={loading} style={{ marginTop:8, padding:'14px', borderRadius:10, border:'none', background:loading?C.border:`linear-gradient(135deg,${C.goldDk},${C.gold})`, color:loading?C.muted:'#06060e', fontSize:15, fontWeight:800, cursor:loading?'default':'pointer', fontFamily:"'Tajawal',sans-serif" }}>
        ...
        {/* STEP 2 — أسئلة قابلة للتعديل والحذف والإضافة */}
        {step===2 && (
          <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:16, padding:32 }}>
            <h2 style={{ fontSize:20, fontWeight:800, marginBottom:6 }}>أسئلة المقابلة</h2>
            <p style={{ fontSize:13, color:C.muted, marginBottom:24 }}>راجع الأسئلة وعدّلها أو احذفها أو أضف أسئلة جديدة</p>
            ...
            {/* زر إضافة سؤال */}
            <button onClick={addQuestion} style={{ width:'100%', padding:'11px', borderRadius:10, border:`2px dashed ${C.border}`, background:'transparent', color:C.muted, fontSize:14, cursor:'pointer', fontFamily:"'Tajawal',sans-serif", marginBottom:20, transition:'all .2s' }}
            ...
            <div style={{ display:'flex', gap:12 }}>
              <button onClick={() => setStep(1)} style={{ padding:'12px 24px', borderRadius:10, border:`1px solid ${C.border}`, background:'transparent', color:C.muted, fontSize:14, cursor:'pointer', fontFamily:"'Tajawal',sans-serif" }}>← تعديل التفاصيل</button>
              <button onClick={publishJob} disabled={loading||questions.length===0} style={{ flex:1, padding:'12px', borderRadius:10, border:'none', background:loading?C.border:`linear-gradient(135deg,${C.goldDk},${C.gold})`, color:loading?C.muted:'#06060e', fontSize:15, fontWeight:800, cursor:loading?'default':'pointer', fontFamily:"'Tajawal',sans-serif" }}>
