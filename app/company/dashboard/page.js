...
export default function CompanyDashboard() {
  ...
  return (
    <div style={{ minHeight:'100vh', background:'var(--bg)', fontFamily:"'IBM Plex Sans Arabic', sans-serif", color:'var(--text)' }}>
      <link href="https://fonts.googleapis.com/css2?family=IBM+Plex+Sans+Arabic:wght@300;400;500;600;700&family=Cormorant+Garamond:wght@300;400;600&display=swap" rel="stylesheet"/>
      ...
      {!loading && freeDone && (
        <div style={{ background:'rgba(201,74,74,.08)', borderBottom:'1px solid rgba(201,74,74,.3)', padding:'10px 20px', display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:10 }}>
          ...
          <button onClick={() => alert('Moyasar — قريباً!')} style={{ padding:'6px 16px', borderRadius:8, fontSize:12, fontWeight:700, background:'linear-gradient(135deg,#7a5e28,#c8a04a)', color:'#06060e', border:'none', cursor:'pointer', fontFamily:"'IBM Plex Sans Arabic', sans-serif" }}>
            اشترك الآن
          </button>
        </div>
      )}
