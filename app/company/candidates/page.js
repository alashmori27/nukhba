              ...
              >
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:16 }}>
                  ...
                  <div style={{ display:'flex', flexDirection:'column', gap:8, flexShrink:0 }}>
                    ...
                    {/* زر واتساب */}
                    {phone && (
                      <button onClick={() => contactWhatsapp(phone, p.name||c.name)} style={{ padding:'7px 14px', borderRadius:8, fontSize:12, fontWeight:700, border:'none', background:'#25D366', color:'#fff', cursor:'pointer', fontFamily:"'Tajawal',sans-serif", display:'flex', alignItems:'center', gap:5, justifyContent:'center' }}>
                        واتساب 💬
                      </button>
                    )}
                    ...
                    {/* لو ما في معلومات تواصل */}
                    {!phone && !email && (
                      <span style={{ fontSize:11, color:C.muted, textAlign:'center' }}>لا يوجد تواصل</span>
                    )}
                  </div>
                </div>
                ...
                {/* ملخص */}
                {p.summary_ar && (
                  <div style={{ fontSize:13, color:C.muted, lineHeight:1.75, margin:'12px 0', paddingTop:12, borderTop:`1px solid ${C.border}`, display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical', overflow:'hidden' }}>
                    {p.summary_ar}
                  </div>
                )}
                ...
                {selected?.id===c.id && (
                  <div style={{ marginTop:16, paddingTop:16, borderTop:`1px solid ${C.border}` }}>
                    ...
                    {/* معلومات التواصل */}
                    <div style={{ background:C.surface, borderRadius:10, padding:'14px 16px', marginBottom:16, display:'flex', gap:20, flexWrap:'wrap' }}>
                      <div style={{ fontSize:12, color:C.muted }}>معلومات التواصل</div>
                    ...
                    {/* أزرار تواصل في الملف الموسع */}
                    <div style={{ display:'flex', gap:10, marginTop:16, flexWrap:'wrap' }}>
                      {phone && (
                        <button onClick={() => contactWhatsapp(phone, p.name||c.name)} style={{ flex:1, padding:'10px', borderRadius:10, fontSize:13, fontWeight:700, border:'none', background:'#25D366', color:'#fff', cursor:'pointer', fontFamily:"'Tajawal',sans-serif" }}>
                          💬 تواصل عبر واتساب
                        </button>
                      )}
                      {email && (
                        <button onClick={() => contactEmail(email, p.name||c.name)} style={{ flex:1, padding:'10px', borderRadius:10, fontSize:13, fontWeight:700, border:`1px solid ${C.gold}`, background:'transparent', color:C.gold, cursor:'pointer', fontFamily:"'Tajawal',sans-serif" }}>
                          📧 تواصل عبر إيميل
                        </button>
                      )}
                    </div>
